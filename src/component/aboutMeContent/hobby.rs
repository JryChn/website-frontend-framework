...
pub fn Hobby(hobbys: Vec<HobbyContent>) -> Element {
    ...
    hobbys.iter().for_each(|h| {
        match flag {
            0 => {
                render_block.push(rsx! {
                LeftImageBlock{bg_color:"bg-[rgb(91,128,86)]",hobby:h.clone()}});
            }
            1 => {
                render_block.push(rsx! {
                RightImageBlock{bg_color:"bg-[rgb(108,131,175)",hobby:h.clone()}});
            }
            _ => {
                render_block.push(rsx! {
                LeftImageBlock{bg_color:"bg-[rgb(173,178,131)]",hobby:h.clone()}});
            }
        }
        flag = (flag + 1) % 3;
    });
